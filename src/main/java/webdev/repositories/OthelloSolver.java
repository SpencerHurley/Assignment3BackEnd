...
public class OthelloSolver {
	...
	static float evaluationFunction(int[][] board) {
        ...
        int whiteCount = 0;
	  	...
	  	return (float) whiteCount - (float) blackCount;	
	}
	...
	static float minimax_value(int board[][], boolean whiteTurn, int searchDepth, float alpha, float beta) {
        if (searchDepth <= 0) {
        	...
        	if (whiteTurn) {
        		return this.max_value(board, searchDepth - 1);
        	} else {
        		return this.min_value(board, searchDepth - 1);
        	}
        }
	}

	max_value(int board[][], int searchDepth) {
		if (checkGameOver(board) = NOBODY && searchDepth >= 0) {
			int curMax = -1000;
			ArrayList<Move> whiteLegalMoves = generateLegalMoves(board, true);
			for (Move m : whiteLegalMoves) {
				curMax = Math.max(curMax, evaluationFunction(m, min_value(this.play(board, m, true), searchDepth - 1)));
			}
			return curMax;
		} else {
			return findWinner(board);
	...
	min_value(int board[][], int searchDepth) {
		if (checkGameOver(board) = NOBODY && searchDepth >= 0) {
			int curMax = 1000;
			ArrayList<Move> blackLegalMoves = generateLegalMoves(board, false);
			for (Move m : blackLegalMoves) {
				curMax = Math.min(curMax, evaluationFunction(m, max_value(this.play(board, m, true), searchDepth - 1)));
			}
			return curMax;
		} else {
			return findWinner(board);
